...
class Validator:
    def __init__(self, config, mapping, dictionary_dfs):
        ...
        # IMPORTANTE: se hace una copia de cada DataFrame del diccionario
        # para evitar que las mutaciones internas (normalización de columnas)
    ...
    def validate_all(self, source_df: pd.DataFrame) -> ErrorCollector:
        ...
        for _, row in df_country.iterrows():
            ...
            if npc_norm not in country_map:
                country_map[npc_norm] = set()
        ...
        for index, row in source_df.iterrows():
            ...
            # Filtrar filas inútiles (TOTAL, ALL, NaN)
            if pd.isna(raw_npc) or pd.isna(raw_param) or \
               "TOTAL" in str(raw_npc).strip().upper() or \
               "ALL" in str(raw_npc).strip().upper():
                continue
            ...
            if norm_param not in valid_params_map:
                continue
            if norm_npc not in country_map:
                self.errors.add_error(
                    country_name, row_num, self.mapping.nameplate_column,
                    str(raw_npc), f"Model '{raw_npc}' not in dictionary."
                )
                continue
            if norm_trim not in country_map[norm_npc]:
                self.errors.add_error(
                    country_name, row_num, self.mapping.trim_column,
                    str(raw_trim), f"TRIM '{raw_trim}' not valid for '{raw_npc}'."
                )
        return self.errors
